...
int localiza(tipoDato x, Lista lista){
	for(int i=primero(lista);i<fin(lista);i=siguiente(i,lista)){
		tipoDato aux = recupera(i,lista);
		if(strncmp(aux.dato.fecha_fallecimiento, x.dato.fecha_fallecimiento, sizeof(aux.dato.fecha_fallecimiento) == 0)) {
			if(strncmp(aux.dato.fecha_nacimiento, x.dato.fecha_nacimiento, sizeof(aux.dato.fecha_nacimiento) == 0)) {
				if(strncmp(aux.dato.nombre, x.dato.nombre, sizeof(aux.dato.nombre) == 0)) {
					return i;
				}
			}
		}
